...
DEFAULT_LEVEL = 3  # 59 MB RAM; -8 needs ~1.6 GB
...
def parse_level(argv: list[str] | None = None) -> int:
    argv = list(sys.argv[1:] if argv is None else argv)
    if not argv:
        return DEFAULT_LEVEL
    token = argv[0].lstrip("-")
    if not token.isdigit():
        raise SystemExit(f"usage: paq_probe.py [0-8]  (default {DEFAULT_LEVEL})")
    level = int(token)
    if not 0 <= level <= 8:
        raise SystemExit("paq8l level must be 0..8")
    return level
